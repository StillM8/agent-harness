...
| `codex`          | `opencode` / `codex`                | Yes                    | Stage + explicit extension native install | Yes      | Yes / yes / yes              | `extension` via Codex CLI        | Project-local `AGENTS.md`, `.agents/skills`, `plugins/agent-harness`, `.codex/agents`, `.codex/agent-harness`, and structured native config when supplied | Global Codex config, plugin caches, automations, remote connections, and sandbox settings are not modified. Current plugin manifests do not synthesize hook registration; MCP, hook, and rules activation requires explicit structured Codex-native config and trusted-project review.                                                                                                                                                                                                                                                         |
