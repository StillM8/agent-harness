...
export async function resolveAssetContent(options: {
  ...
  const asset = await readJsonFileOrNull<AssetCatalogEntry>(join(options.activationRoot, sanitizeAssetId(options.assetId), "asset.json"))
  if (!asset) {
    return null
  }

  const mirroredContent = await readTextFileOrNull(join(options.activationRoot, sanitizeAssetId(options.assetId), "content.txt"))
...
function shouldUseMirroredContent(asset: AssetCatalogEntry, mirroredContent: string | null): boolean {
  if (!mirroredContent) {
    return false
  }
  ...
  if (asset.source.sourceKind === "local-directory" || asset.source.sourceKind === "local-manifest") {
    return true
  }
...
async function resolveGitHubAssetContent(asset: AssetCatalogEntry): Promise<string | null> {
  const repoInfo = parseGitHubRepoInfo(asset)
  if (!repoInfo || !asset.evidence.filePath) {
    return null
  }

  const rawUrlCandidates = buildGitHubRawUrlCandidates(repoInfo.owner, repoInfo.repo, asset.evidence.filePath)
...
async function resolveOfficialSkillPageContent(asset: AssetCatalogEntry): Promise<string | null> {
...
function buildMetadataFallback(asset: AssetCatalogEntry, mirroredContent?: string): string {
  ...
  if (mirroredContent) {
    lines.push("", "## Mirrored content snapshot", "", "```json", mirroredContent, "```")
  }

  return `${lines.join("\n").trim()}\n`
}

function parseGitHubRepoInfo(asset: AssetCatalogEntry): { owner: string; repo: string } | null {
  const urlsToCheck = [asset.source.originUrl, asset.evidence.rootPath].filter((value): value is string => typeof value === "string")
...
function buildGitHubRawUrlCandidates(owner: string, repo: string, filePath: string): string[] {
  const possibleBranches = ["main", "master"]
  return possibleBranches.map((branch) => `https://raw.githubusercontent.com/${owner}/${repo}/${branch}/${filePath}`)
}
