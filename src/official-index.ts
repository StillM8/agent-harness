...
export async function fetchOfficialIndexPageContent(url: string): Promise<string | null> {
  try {
...
export function buildOfficialIndexAssetStatus(authorityTier: AuthorityTier): AssetStatus {
...
function extractOfficialIndexPageSummary(html: string, url: string): string {
  ...
  const lines = [
    ...
    "",
    "## Core Concept",
    skillSummary ?? description ?? "No concise summary was available from the official index page.",
    whyItHelps ? "" : "",
...
function buildKeyPoints(input: {
  ...
  if (input.installCommand) {
    points.push("- Installable directly with the official `npx skills add` flow.")
  }

  if (input.githubUrl) {
    points.push("- Backed by a linked GitHub repository for deeper inspection and updates.")
  }
...
function extractTitle(html: string): string | null {
  ...
  return rawTitle?.replace(/\s+—\s+Agent Skills\s+\|\s+officialskills\.sh$/iu, "") ?? null
}

function extractMetaDescription(html: string): string | null {
  const descriptionMatch = /<meta\s+name="description"\s+content="([^"]+)"/iu.exec(html)
  return cleanHtmlText(descriptionMatch?.[1] ?? "")
}

function extractInstallCommand(html: string): string | null {
  const commandMatch = /npx skills add[^<]{0,300}?--skill\s+[a-z0-9-]+/iu.exec(html)
  return cleanHtmlText(commandMatch?.[0] ?? "")
}

function extractGitHubUrl(html: string, installCommand: string | null): string | null {
  const installCommandRepositoryUrl = /npx skills add\s+(https:\/\/github\.com\/[^\s]+)\s+--skill/iu.exec(installCommand ?? "")?.[1]
  if (installCommandRepositoryUrl) {
    return installCommandRepositoryUrl
  }

  const viewOnGitHubMatch = /<a href="(https:\/\/github\.com\/[^"]+)"[^>]*>.*?View on GitHub<\/a>/iu.exec(html)
...
function extractSectionContent(heading: string, html: string): string | null {
  ...
  const sectionMatch = new RegExp(`<h2[^>]*>${escapedHeading}<\\/h2>([\\s\\S]*?)<\\/section>`, "iu").exec(html)
  if (sectionMatch?.[1]) {
    return sectionMatch[1]
  }

  const fallbackMatch = new RegExp(`<h3[^>]*>${escapedHeading}<\\/h3>([\\s\\S]*?)(?:<h[23]|<\\/section>)`, "iu").exec(html)
