...
import type { BundleLock, InstallProgressState, MirrorAcquireState } from "./types.js"
...
async function cleanState(projectRoot: string): Promise<void> {
  ...
  console.log(`Cleaned install/activate transient state under ${toPosixPath(projectRoot)}`)
}
...
async function acquireAllMirrorBatches(projectRoot: string): Promise<void> {
  ...
  for (let batchIndex = 0; batchIndex < maxBatches; batchIndex += 1) {
    await runMirror(["acquire", "--batch-size", batchSize], projectRoot, projectRoot)
...
async function installAllBundleBatches(projectRoot: string): Promise<void> {
  ...
  for (const bundleId of bundleIds) {
    for (let batchIndex = 0; batchIndex < maxBatchesPerBundle; batchIndex += 1) {
      await runInstall(["bundle", "--bundle", bundleId, "--batch-size", batchSize], projectRoot, projectRoot)
...
async function discoverBundleIds(projectRoot: string): Promise<string[]> {
  ...
  for (const bundleFileName of BUNDLE_LOCK_FILE_NAMES) {
    const bundleLock = await readJsonFileOrNull<BundleLock>(join(projectRoot, "mirror", "bundles", bundleFileName))
