...
async function generateMirrorPlan(projectRoot: string): Promise<void> {
  const policy = await readJsonFile<MirrorPolicy>(join(projectRoot, "mirror", "policy.json"))
  const demandProfile = await readJsonFileOrNull<DemandProfile>(join(projectRoot, "discover", "output", "demand-profile.json"))
  const sourceIndex = await readJsonFileOrNull<SourceIndex>(join(projectRoot, "discover", "output", "source-index.json"))
  const catalogEntries = await readJsonLinesFile<AssetCatalogEntry>(join(projectRoot, "discover", "catalog.assets.jsonl"))
  ...
  const mirrorEligibleEntries = catalogEntries.filter((entry) => entry.status.mirrorEligible)
...
function buildNextActions(
  ...
  if (!demandProfile) {
    nextActions.push("Run discover demand-profile to capture current-directory portfolio signals.")
  }

  if (!sourceIndex) {
    nextActions.push("Run discover sources to summarize enabled discovery sources.")
  }

  if (catalogEntries === 0) {
    nextActions.push("Run discover catalog to harvest local manifest and local directory sources into discover/catalog.assets.jsonl.")
  }

  if (catalogEntries > 0 && mirrorEligibleEntries === 0) {
    nextActions.push("Review catalog statuses and enable mirror eligibility only for approved local assets.")
  }

  if (selectedCatalogEntries === 0) {
    nextActions.push("Create discover/output/catalog.selected.jsonl after canonical selection and promotion rules are applied to catalog entries.")
  }

  if (nextActions.length === 0) {
    nextActions.push("Resolve exact artifact versions for canonical selected assets before writing mirror locks.")
    nextActions.push("Mirror only canonical or explicitly approved alternative assets into the inert mirror store.")
  }
...
async function generateBundleLocks(projectRoot: string): Promise<void> {
  const policy = await readJsonFile<MirrorPolicy>(join(projectRoot, "mirror", "policy.json"))
  const selectedEntries = await readJsonLinesFile<AssetCatalogEntry>(join(projectRoot, "discover", "output", "catalog.selected.jsonl"))
  const mirrorEligibleEntries = selectedEntries.filter((entry) => entry.status.mirrorEligible)

  for (const bundleTemplate of policy.bundleTemplates) {
    const bundleAssets = mirrorEligibleEntries
      .filter((entry) => shouldIncludeEntryInBundle(entry, bundleTemplate.id, bundleTemplate.host, bundleTemplate.assetKinds))
      .map((entry) => createBundleLockAsset(entry, bundleTemplate.id))
      .filter((asset, index, assets) => assets.findIndex((candidate) => candidate.assetId === asset.assetId) === index)
    ...
    await writeJsonFile(join(projectRoot, "mirror", "bundles", `${bundleTemplate.id}.lock.json`), bundleLock)
  }

  console.log(`Bundle locks written to ${toPosixPath(join(projectRoot, "mirror", "bundles"))}`)
}

async function acquireMirrorArtifacts(projectRoot: string, args: string[]): Promise<void> {
  const selectedEntries = await readJsonLinesFile<AssetCatalogEntry>(join(projectRoot, "discover", "output", "catalog.selected.jsonl"))
  const mirrorEligibleEntries = selectedEntries.filter((entry) => entry.status.mirrorEligible)
  const batchSize = Number(getOptionValue(args, "--batch-size") ?? process.env.AGENT_HARNESS_MIRROR_BATCH_SIZE ?? "120")
  const existingMirrorIndexEntries = await readJsonLinesFile<MirrorIndexEntry>(join(projectRoot, ...MIRROR_INDEX_OUTPUT_PATH))
  const existingMirrorIdsByAssetId = new Map(existingMirrorIndexEntries.map((entry) => [entry.assetId, entry.mirrorId]))
  const unresolvedEntries = mirrorEligibleEntries.filter((entry) => !existingMirrorIdsByAssetId.has(entry.id))
  ...
  const mergedMirrorIndexEntries = mergeMirrorIndexEntries(existingMirrorIndexEntries, newMirrorIndexEntries)
  await writeJsonLinesFile(join(projectRoot, ...MIRROR_INDEX_OUTPUT_PATH), mergedMirrorIndexEntries)
  ...
  await writeMirrorAcquireState(projectRoot, {
    ...
    mirroredCount: mirrorEligibleEntries.filter((entry) => mergedMirrorIndexEntries.some((mirrorEntry) => mirrorEntry.assetId === entry.id)).length,
    remainingCount: Math.max(0, unresolvedEntries.length - entriesToAcquire.length),
    lastBatchAssetIds: entriesToAcquire.map((entry) => entry.id)
  })

  console.log(`Mirror artifacts acquired under ${toPosixPath(join(projectRoot, "mirror"))}`)
}
...
function createBundleLockAsset(entry: AssetCatalogEntry, bundleId: string): BundleLockAsset {
  return {
    assetId: entry.id,
    mirrorId: `unresolved:${entry.id}`,
    projectionType: determineProjectionType(entry, bundleId),
    activationEligible: false,
    notes: "Resolve exact upstream artifact and replace unresolved mirrorId during raw mirror acquisition."
  }
}

function determineProjectionType(entry: AssetCatalogEntry, bundleId: string): string {
  if (bundleId === "shared-mcp") {
...
function determineMirrorStatus(entry: AssetCatalogEntry): MirrorIndexEntry["status"] {
  if (entry.risk.level === "high" && entry.source.authorityTier !== "official-first-party") {
    return "quarantined"
  }
...
async function resolveBundleLocks(projectRoot: string, mirrorIndexEntries: MirrorIndexEntry[]): Promise<void> {
  const mirrorIdByAssetId = new Map(mirrorIndexEntries.map((entry) => [entry.assetId, entry.mirrorId]))
  const bundlePaths = [
...
async function writeMirrorAcquireState(projectRoot: string, state: MirrorAcquireState): Promise<void> {
  await writeJsonFile(join(projectRoot, ...MIRROR_ACQUIRE_STATE_OUTPUT_PATH), state)
}
...
function mergeMirrorIndexEntries(
  ...
  const byAssetId = new Map(existingEntries.map((entry) => [entry.assetId, entry]))
  for (const entry of newEntries) {
    byAssetId.set(entry.assetId, entry)
  }

  return [...byAssetId.values()].sort((left, right) => left.assetId.localeCompare(right.assetId))
}

function buildGitHubCachePath(entry: AssetCatalogEntry, projectRoot: string): string | null {
  if (!entry.source.sourceId || !entry.source.originUrl.includes("github.com")) {
    return null
  }
...
async function pathLooksReadable(filePath: string): Promise<boolean> {
  return readTextFileOrNull(filePath).then((content) => content !== null).catch(() => false)
}
...
function countEntriesByHost(entries: AssetCatalogEntry[]): Record<string, number> {
...
function countEntriesByAssetKind(entries: AssetCatalogEntry[]): Record<string, number> {
  const counts: Record<string, number> = {}
