...
async function writeCodexAgentProfiles(
  ...
  for (const [fileName, priorRecord] of previousByFileName) {
    ...
    // A user-owned profile (edited after apply) is never orphan-cleaned.
    if (priorRecord.userOwned === true) continue;
...
async function removeCodexAgentProfiles(workspaceRoot: string): Promise<void> {
  ...
  for (const record of records) {
    ...
    // reset — the user's file is preserved as-is.
    if (record.userOwned === true) continue;
