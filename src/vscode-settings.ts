...
import { ensureDirectory, pathExists, readTextFileOrNull, writeTextFile } from "./files.js"

export async function readVsCodeSettings(settingsPath: string): Promise<Record<string, unknown>> {
  ...
  if (errors.length > 0) {
    throw new Error(`VS Code settings.json contains JSONC parse errors and could not be safely patched: ${JSON.stringify(errors)}`)
  }
